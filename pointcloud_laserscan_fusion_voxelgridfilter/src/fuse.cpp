...
class Talker
{
  ...
  // Solve all of perception here...
    ...
    if (!(projected_laser.header.frame_id == cloud_msg->header.frame_id))
    {
      try
      {
        cloud.reset(new sensor_msgs::PointCloud2);
        tf2_->transform(*cloud_msg, *cloud, projected_laser.header.frame_id, ros::Duration(tolerance_));
        cloud_out = cloud;
      }
    ...
    ROS_INFO("Number of points in the callback: %lu",pc_count);
